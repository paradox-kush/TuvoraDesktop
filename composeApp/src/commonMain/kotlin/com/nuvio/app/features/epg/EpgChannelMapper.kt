...
internal object EpgNorm {
    ...
    fun coreNorm(s: String): String {
        ...
        val toks = stripped.split(" ").filter { it.isNotEmpty() && it !in QUALITY }
            ...
            // of the table, so only position 0 is exempt. Measured: kills both false positives
            // for 8 matches out of 3,524.
...
internal class EpgChannelIndex private constructor(
     ...
     *
     * `UK: ITV +1` -> `ITV1` and `Animal Planet +1` -> `Animal Planet` were real hits on the
     * user's panel. A timeshift channel carries the SAME programmes an hour later, so this is
     * the worst kind of wrong match: every title looks plausible and every time is an hour off.
     *
     * Deliberately one-directional — a non-timeshift panel channel is NOT blocked just because
     * the EPG id happens to carry a `+1` alias among its display names, which was blocking a
     * correct `Tennis Channel` match when the rule ran both ways.
     */
