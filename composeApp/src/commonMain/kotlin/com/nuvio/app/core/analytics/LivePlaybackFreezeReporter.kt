 ...
 * the process stays alive so `app_exit` never fires, and no exception is thrown so
 * `$exception` never fires.
 *
 * So this reports nothing during healthy playback. One event is emitted per freeze, and only
 * once the freeze *resolves* — either it cleared on its own, or playback was torn down while
 ...
 */
class LivePlaybackFreezeReporter(
    private val capture: (String, Map<String, Any>) -> Unit = AnalyticsSink::capture,
) {
    ...
    data class Profile(
        ...
        val iptvKind: String,
        /** Which live surface the viewer was on: the docked Live TV screen or the full player. */
        val surface: String,
        /** Null for engines that do not express buffering as thresholds; omitted from the event. */
        val buffers: LivePlaybackBufferProfile.Values? = LivePlaybackBufferProfile.current,
    )
    ...
    /**
     * Whether a freeze is open. A reconnect tears playback down and builds it back up, so the
     ...
     * on a freeze (reconnect) as well as record it. Cheap; safe to call from the snapshot callback.
     */
    fun onSample(
        nowMs: Long,
        positionMs: Long,
        ...
        when (decision) {
            ...
            is LivePlaybackFreezePolicy.Decision.Recover -> {
                ...
                emit(nowMs = nowMs, recovered = true, bufferedAheadAtResolveMs = bufferedAheadMs)
                resetFreezeState()
                markAdvanced(nowMs, positionMs)
            }
        }
    ...
    private fun emit(nowMs: Long, recovered: Boolean, bufferedAheadAtResolveMs: Long) {
        ...
        val properties = buildMap<String, Any> {
            ...
            activeProfile.buffers?.let { buffers ->
                put("min_buffer_ms", buffers.minBufferMs)
                put("max_buffer_ms", buffers.maxBufferMs)
                put("buffer_for_playback_ms", buffers.bufferForPlaybackMs)
                put("buffer_for_playback_after_rebuffer_ms", buffers.bufferForPlaybackAfterRebufferMs)
            }
    ...
    companion object {
        ...
        /** Backstop across channel changes within one reporter. */
        ...
        fun iptvKindOf(contentId: String?): String = when {
