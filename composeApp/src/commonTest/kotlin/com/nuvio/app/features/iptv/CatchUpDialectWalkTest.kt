...
import com.nuvio.app.features.iptv.CatchUpDialectWalk.FailureKind
import com.nuvio.app.features.iptv.CatchUpDialectWalk.Step
...
/** KMP port of NuvioTV's CatchUpDialectWalkTest — same machine, same guards. */
class CatchUpDialectWalkTest {
    ...
    private fun next(step: Step): CatchUpDialectWalk.Attempt {
        assertTrue(step is Step.Next, "expected Next but was $step")
    ...
    fun `the walk terminates after every dialect failed`() {
        ...
        while (step is Step.Next) {
            attempts++
            assertTrue(attempts <= 8, "the ladder must terminate, not loop")
            step = walk.onFailure(step.attempt.token, FailureKind.TRANSPORT)
        }
        assertEquals(Step.Unavailable, step)
