...
class StalkerMagPresetsTest {
    ...
    fun `presets are distinct in the fields portals fingerprint`() {
        ...
        assertEquals(identities.size, identities.distinct().size, "a duplicate identity would waste a rung")
        assertEquals(StalkerMagPresets.LADDER.size, StalkerMagPresets.LADDER.map { it.id }.distinct().size, "preset ids must be unique — they are persisted")
    }
