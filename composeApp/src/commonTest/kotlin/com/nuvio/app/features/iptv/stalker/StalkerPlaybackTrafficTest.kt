...
class StalkerPlaybackTrafficTest {
    ...
    @BeforeTest fun setUp() = StalkerPlaybackTraffic.resetForTests()
    @AfterTest fun tearDown() = StalkerPlaybackTraffic.resetForTests()
     ...
     *
     * Field report (S24, 2026-08-16): scrolling a Stalker portal queues dozens of 14-row
     * get_ordered_list calls behind the 2-permit gate; switching to an Xtream provider on the SAME
     * host then hangs — the abandoned backlog keeps draining at the throttled host's pace, ahead of
     * everything the user is now looking at. A queued browse call from before the switch must be
     * dropped when its turn finally comes; playback-critical calls are never dropped (a replay
     * resolving mid-switch must not lose its create_link).
     */
    @Test
