...
/**
 * Pins the tier rules and the pressure-escalation semantics (WP1).
 *
...
class MemoryTierPolicyTest {
    ...
    @Test
    fun `ios probe maps physical memory to tiers`() {
        // <=2GB LOW, <=3GB MID, else HIGH.
        assertEquals(MemoryTier.LOW, MemoryTierPolicy.iosTier(1 * gib))
        assertEquals(MemoryTier.LOW, MemoryTierPolicy.iosTier(2 * gib))
        assertEquals(MemoryTier.MID, MemoryTierPolicy.iosTier(3 * gib))
        assertEquals(MemoryTier.HIGH, MemoryTierPolicy.iosTier(4 * gib))
        assertEquals(MemoryTier.HIGH, MemoryTierPolicy.iosTier(6 * gib))
    }

    @Test
    fun `desktop tier is always high`() {
        assertEquals(MemoryTier.HIGH, MemoryTierPolicy.desktopTier())
    }
     ...
     *
     * Measured on a 2 GB Onn 4K TV box (2026-08-16, v1.4.30): the catalog index build held a worker
     * at 97% CPU for minutes and the hub's own reads — which come from the SAME index DB — queued
     * behind its write transaction, so movies "didn't load" and rows never filled. The batch was
     * 5,000 rows, each costing an UPDATE-or-INSERT plus one INSERT per normalized key: roughly
    ...
    fun `index batch size follows the tier`() {
        assertEquals(100, MemoryTierPolicy.indexBatchSize(MemoryTier.LOW), "a 1GB box must not hold 5k rows of writes")
