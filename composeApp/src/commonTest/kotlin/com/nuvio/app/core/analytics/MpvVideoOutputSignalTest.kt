...
import com.nuvio.app.core.analytics.LivePlaybackFreezePolicy.Decision
import com.nuvio.app.core.analytics.LivePlaybackFreezePolicy.Input
import com.nuvio.app.core.analytics.LivePlaybackFreezePolicy.Kind
import com.nuvio.app.core.analytics.LivePlaybackFreezePolicy.PlaybackState
 ...
 * Onn 4K: the count sat still for 60s while a channel played fine on mpv), so the count plateaued
 * on healthy steady-state playback exactly as it does on a real freeze — the policy could not tell
 ...
 * approaches and prove the new one does not trip on a healthy channel yet still catches a real one.
 */
class MpvVideoOutputSignalTest {
    ...
    fun `constant fps advances the tick every sample`() {
        ...
        assertEquals(10L, ticks, "steady 25fps must keep advancing the tick")
    }
    ...
    fun `zero fps holds the tick so a freeze stays observable`() {
        ...
        assertEquals(7L, ticks, "a stopped filter chain must hold the tick")
    }

    @Test
    fun `a decayed sub-one-fps estimate counts as frozen`() {
        assertEquals(7L, MpvVideoOutputSignal.advance(7L, 0.4), "0.4fps is below the liveness floor")
    }
